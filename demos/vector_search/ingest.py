#!/usr/bin/env python3
"""Recipe vector ingestion: OpenSearch vector chunks plus filterable metadata.
...
* Each chunk carries structured recipe metadata as keyword fields for exact filters.
...
Embeddings handle semantic intent. Keyword metadata handles allergy and label filters.
Asking vectors to remember that "not peanuts" means "not peanuts" is how software
quietly becomes a snack-themed liability hearing.
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--vector-chunk-size",
        type=int,
        default=2048,   # default=1024, alt=8192
...
# without metadata
# def build_recipe_vector_chunks(
#     recipe: RecipeRecord,
#     page_text: str,
#     *,
#     chunk_size: int,
#     chunk_overlap: int,
# ) -> List[str]:
#     """Create vector chunks from recipe page text only.

#     Metadata is intentionally excluded from vector chunks so embeddings are
#     generated only from the recipe page content.
#     """
