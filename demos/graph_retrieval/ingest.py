...
* Neo4j stores structured recipe metadata and explicit allergy warning nodes.
* URL text extraction is attempted for each recipe with bounded exponential backoff.
* Recipes with unavailable or unreadable pages are skipped so the graph does not
  contain stale partial records.
* The full extracted recipe text is stored on ``Recipe.content``.
* Optional content chunks are stored as ``RecipeChunk`` nodes for graph-side
  inspection and future text workflows.

Allergy inclusion/exclusion belongs in the graph, where facts have a fighting
chance against wishful thinking and string filters do not pretend to be cuisine.
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=1024,   # default=1024, alt=8192
        help="Target maximum characters per graph content chunk.",
    )
    parser.add_argument(
        "--chunk-overlap",
        type=int,
        default=256,    # default=256, alt=512
...
def build_recipe_content_chunks(recipe: RecipeRecord, page_text: str, *, chunk_size: int, chunk_overlap: int) -> List[str]:
    """Create graph content chunks with recipe/allergy metadata available in every chunk."""
